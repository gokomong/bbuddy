---
name: language
description: Set or view the bbuddy UI language (English / Korean)
---

# /bbuddy:language — Set the bbuddy UI language

Use this skill when the user types `/bbuddy:language [en|ko]`.

## Role

bbuddy's wizard prompts, error messages, reaction templates, and ambient
idle phrases are localized into English (default) and Korean. Call
`bbuddy_language` to read or change the active language.

## Calls

```
bbuddy_language({})                 // show current language
bbuddy_language({ lang: "en" })     // switch to English
bbuddy_language({ lang: "ko" })     // switch to Korean
```
