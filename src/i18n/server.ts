...
export const SERVER_STRINGS = {
  en: {
    // bbuddy_create / bbuddy_evolve — AI delegation prompt
    ...
    existingCompanion: 'A companion already exists. Call bbuddy_respawn first, then try again.',
    ...
    noCompanionToSave: 'No companion to save. Run bbuddy_hatch or bbuddy_create first.',
    slotSaved: (name: string, slot: string) => `💾 "${name}" saved to slot "${slot}".`,
    slotMissing: (slot: string) => `Slot "${slot}" not found. Run bbuddy_list to see available slots.`,
    ...
    noSlotsHeader: 'Saved bbuddy slots:',
    noSlots: 'No saved slots yet. Use bbuddy_save to save the current companion.',
    ...
    slotListFooter: 'Use bbuddy_summon { slot: "<name>" } to load a slot.',
    slotAutoBackupTag: ' (auto-backup)',
    slotCorrupted: '(corrupted)',

    languageSet: (lang: Lang) => `Language set to ${lang === 'en' ? 'English' : 'Korean'}.`,
    languageCurrent: (lang: Lang) => `Current language: ${lang === 'en' ? 'English' : 'Korean'} (use bbuddy_language { lang: "en" | "ko" } to change)`,
  ...
  ko: {
    ...
    existingCompanion: '이미 companion이 있습니다. bbuddy_respawn으로 해제한 뒤 다시 시도하세요.',
    ...
    noCompanionToSave: '저장할 companion이 없습니다. bbuddy_hatch 또는 bbuddy_create로 먼저 만드세요.',
    slotSaved: (name: string, slot: string) => `💾 "${name}"이(가) 슬롯 "${slot}"에 저장되었습니다.`,
    slotMissing: (slot: string) => `슬롯 "${slot}"이(가) 없습니다. bbuddy_list로 사용 가능한 슬롯을 확인하세요.`,
    ...
    noSlotsHeader: '저장된 bbuddy 슬롯:',
    noSlots: '저장된 슬롯이 없습니다. bbuddy_save로 현재 companion을 저장하세요.',
    slotLine: (slot: string, tag: string, name: string, species: string, level: number, savedAt: string) =>
      `  • ${slot}${tag} — ${name} (${species}) Lv.${level}  · ${savedAt}`,
    slotListFooter: 'bbuddy_summon { slot: "<name>" } 으로 불러오세요.',
    slotAutoBackupTag: ' (자동 백업)',
    slotCorrupted: '(손상됨)',

    languageSet: (lang: Lang) => `언어가 ${lang === 'ko' ? '한국어' : '영어'}로 설정되었습니다.`,
    languageCurrent: (lang: Lang) => `현재 언어: ${lang === 'ko' ? '한국어' : '영어'} (변경하려면 bbuddy_language { lang: "en" | "ko" })`,
