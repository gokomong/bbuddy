...
// UI text (label + description) is localized; bioTemplate / observerTone
// stay in English because they feed the LLM system prompt and the English
// wording is what's been tuned for the reaction pipeline.
const PRESET_LABELS = {
  en: {
    tsundere:   { label: 'Tsundere',   description: '"Not that I was worried... good job, I guess."' },
    passionate: { label: 'Passionate', description: '"WOAH!! AMAZING!! Total legend!!"' },
    cold:       { label: 'Cold',       description: '"Build succeeded. Next task."' },
    prankster:  { label: 'Prankster',  description: '"Error? lol, again?"' },
    sage:       { label: 'Sage',       description: '"Failure is merely the debugger of success..."' },
    custom:     { label: 'Custom',     description: 'Describe the personality yourself' },
  },
  ko: {
    tsundere:   { label: '츤데레',     description: '"별로 도움 안 됐지만... 수고했어"' },
    passionate: { label: '열정적',     description: '"와!! 대박!! 완전 잘했어!!"' },
    cold:       { label: '냉정한',     description: '"빌드 성공. 다음 태스크로."' },
    prankster:  { label: '장난꾸러기', description: '"에러? ㅋㅋㅋ 또?"' },
    sage:       { label: '현자',       description: '"실패는 성공의 디버깅이니라..."' },
    custom:     { label: '커스텀',     description: '직접 성격 설명 입력' },
  },
};

function lbl(id: PersonalityPreset): { label: string; description: string } {
  return PRESET_LABELS[getLang()][id];
}

export const PRESETS: Record<PersonalityPreset, PresetDefinition> = {
  tsundere: {
    id: 'tsundere',
    get label() { return lbl('tsundere').label; },
    get description() { return lbl('tsundere').description; },
  ...
  passionate: {
    id: 'passionate',
    get label() { return lbl('passionate').label; },
    get description() { return lbl('passionate').description; },
  ...
  cold: {
    id: 'cold',
    get label() { return lbl('cold').label; },
    get description() { return lbl('cold').description; },
  ...
  prankster: {
    id: 'prankster',
    get label() { return lbl('prankster').label; },
    get description() { return lbl('prankster').description; },
  ...
  sage: {
    id: 'sage',
    get label() { return lbl('sage').label; },
    get description() { return lbl('sage').description; },
  ...
  custom: {
    id: 'custom',
    get label() { return lbl('custom').label; },
    get description() { return lbl('custom').description; },
